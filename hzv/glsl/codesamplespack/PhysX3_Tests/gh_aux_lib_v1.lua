...
-------------
2013-05-21
	- Updated Orbit camera code with set_lookat(), 
    works with GLSL Hacker 0.6.0+
    [JeGX - jegx@geeks3d.com]
...
function GH_CameraMoveAlongView(camera, dist_x, dist_y, dist_z)
...
function GH_CameraRotateView(camera, pitch, yaw)
  gh_camera.set_yaw(camera, yaw)
  gh_camera.set_pitch(camera, pitch)
end

function GH_CameraStrafe(camera, dist)
	local v = vec3()
	local u = vec3()
